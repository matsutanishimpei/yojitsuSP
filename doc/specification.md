...
erDiagram
    ...
    students {
        TEXT id PK "学籍番号 (主キー、例: 24JZ0100)"
        TEXT name "学生氏名"
...
CREATE TABLE IF NOT EXISTS students (
  id TEXT PRIMARY KEY, -- 学籍番号 (例: 24JZ0128)
...
*   **POST `/login`**
    ...
    *   **リクエストボディ (JSON)**:
        ```json
        { "student_id": "24JZ0128", "parent_birthday": "0000" }
        ```
    *   **レスポンス**:
        *   **成功 (200)**:
            ```json
            { "success": true, "name": "高橋晴" }
            ```
        *   **失敗 (401 / 400)**:
            ```json
...
*   **POST `/admin/students/bulk?admin_id=...`**
    ...
    *   **リクエストボディ (JSON)**:
        ```json
        {
          "csv": "24JZ0100,テスト タロウ,0000\n24JZ0199,テスト ジロウ,0000"
        }
        ```
    *   **レスポンス (200)**:
        ```json
